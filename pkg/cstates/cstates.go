...
func (c *Cstate) Copy(filter Filter) *Cstate {
	...
	for attr, val := range c.attrVal {
		...
		if !filter.Match(NewFilterInput(WithCPU(c.cpu), WithCstateName(c.name), WithAttributeValue(AttrID(attr), *val))) {
			continue
		}
...
func (cs *Cstates) Read(filter Filter) error {
	...
	for _, cpu := range cpus.SortedMembers() {
		if !filter.Match(NewFilterInput(WithCPU(cpu))) {
			continue
		}
		...
		for state, cstateName := range stateName {
			...
			if !filter.Match(NewFilterInput(WithCPU(cpu), WithCstateName(cstateName))) {
				continue
			}
			cstate := NewCstate(cstateName, cpu, state)
			for _, attr := range attributes {
				if !filter.Match(NewFilterInput(WithCPU(cpu), WithCstateName(cstateName), WithAttribute(attr.id))) {
					continue
				}
				...
				if !filter.Match(NewFilterInput(WithCPU(cpu), WithCstateName(cstateName), WithAttributeValue(attr.id, value))) {
					continue
				}
...
func (cs *Cstates) Copy(filter Filter) *Cstates {
	...
	newCs.fs = cs.fs
	for _, cstate := range cs.cstates {
		if !filter.Match(NewFilterInput(WithCPU(cstate.cpu), WithCstateName(cstate.name))) {
			continue
		}
